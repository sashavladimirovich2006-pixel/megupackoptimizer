...
- **Detailed Rationale**:
  - **Visual Overhaul**: Unlike the rest of the application which uses the transparent glass capsule buttons (`MeguButton`), the English/Ukrainian language selectors in [SettingsView.qml](file:///C:/Users/alexa/.gemini/antigravity/scratch/megupackoptimizer/src/qml/views/SettingsView.qml) were implemented using raw `Rectangle` delegates. This resulted in a completely solid amber-orange box (`Theme.accent`) when selected and a solid dark grey block (`Theme.buttonBg`) when unselected.
